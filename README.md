...
（`#2563EB`）、白底工具列與卡片、表格列頭淺灰底深字、分頁式表單頁籤為白底深藍字＋
選取頁籤底部藍線。登入頁為深炭品牌區＋白底卡片，與主程式一致。各模組表單均提供
「匯出」（CSV／Excel），含基本資料維護與唯讀檢視。主畫面儀表板提供可點擊 KPI 卡片
（庫存不足、應收／應付餘額、逾期未收／未付、出貨／進貨、待核准單據等）、近 12 月營收趨勢、
應收帳齡分佈與庫存不足警示；快速入口下方顯示「最近使用」模組捷徑（記錄於本機設定目錄）。
側邊導覽 18 個模組依業務屬性分為四組：營運核心、財務會計、生產作業、系統管理。
「說明」選單的說明中心分三頁籤：快捷鍵總表、模組導覽（與側邊導覽同分類）、常見作業流程。
全域快捷鍵：F1 說明中心、F2 新增、F3 修改、F4 刪除、F5 重整、Ctrl+F 搜尋、Ctrl+K 全域搜尋、Esc 關閉。
會計傳票建檔流程：新增傳票主檔自動帶入編號（YYMMDD＋當日流水），儲存後自動進入明細輸入，
並提供「新增並繼續」連續登錄借貸科目。基礎資料建檔（客戶／廠商／員工／倉庫等通用表格）
新增對話框亦提供「新增並繼續」，儲存後自動開啟下一筆空白表單，連續建檔不需重複按「新增」。
