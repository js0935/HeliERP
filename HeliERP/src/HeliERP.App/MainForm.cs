...
public class MainForm : Form
{
    ...
    public MainForm(DbConfig config, AppUser user)
    {
        ...
        _modules = new (string Group, string Name, string Desc, Action Open, bool Dev)[]
        {
            ...
            ("營運核心", "基本資料", "資料表維護總覽", OpenTableBrowser, false),
            ("財務會計", "收付系統", "應收應付 / 帳款", OpenPaymentModule, false),
            ("財務會計", "應收帳款", "餘額總覽 / 帳齡分析", OpenAccountReceivable, false),
            ("財務會計", "票據系統", "應收／應付票據管理與報表", () => new BillModuleForm().ShowDialog(this), false),
            ("財務會計", "會計系統", "傳票 / 會計科目", () => new AccountingModuleForm().ShowDialog(this), false),
            ("財務會計", "折讓作業", "出貨 / 進貨折讓", OpenDiscountModule, false),
            ("財務會計", "電子發票", "字軌建置 / 自動配號 / 開立紀錄", () => new InvoiceTrackForm().ShowDialog(this), false),
            ("財務會計", "核准中心", "採購 / 訂貨 / 收付單據多層核准", () => new ApprovalForm().ShowDialog(this), false),
            ("財務會計", "薪資系統", "出缺勤 / 薪資計算", () => new PayrollModuleForm().ShowDialog(this), false),
            ("生產作業", "採訂系統", "採購 / 訂單", OpenPoOrderModule, false),
            ("生產作業", "生管系統", "驗貨品檢 / 託運出貨", () => new ProductionModuleForm().ShowDialog(this), false),
            ("生產作業", "維修管理", "叫修 / 維修 / 保固", OpenRepairModule, false),
            ("生產作業", "貨品主檔", "貨品資料維護", OpenProductMaintenance, false),
            ("系統管理", "系統健康檢查", "DB 完整性 / WAL / 備份 / 磁碟自檢", () => new HealthCheckForm(_config).ShowDialog(this), false),
            ("系統管理", "系統設定", "帳號權限 / 公司資料 / 密碼", () => new SystemSettingsForm(_config, _user).ShowDialog(this), false),
        };
